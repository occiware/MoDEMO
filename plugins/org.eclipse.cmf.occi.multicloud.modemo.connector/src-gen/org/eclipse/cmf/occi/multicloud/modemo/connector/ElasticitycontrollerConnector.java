 ...
 * - title: 
 */
public class ElasticitycontrollerConnector extends org.eclipse.cmf.occi.multicloud.modemo.impl.ElasticitycontrollerImpl
{
	/**
	...
	protected double CPUGroupUsage() {
		...
		for(Link link : hg.getLinks()) {
			...
			if(link.getTarget() instanceof Ec2) {
				...
				if (!(inst.getAwsEc2instancePublicIP().isEmpty())) {
					String instanceIp = inst.getAwsEc2instancePublicIP();
					System.out.println("trying to connect to zabbix");
					System.out.println("instance ip " + instanceIp);
					String zabi = zabbix_obj.connect();
					System.out.println("zabbi authentification " +  zabi);
			...
			if(link.getTarget() instanceof Azurevm) {
				...
				if (!(inst.getAzureVmazurePublicIP().isEmpty())) {
					String instanceIp = inst.getAzureVmazurePublicIP();
					System.out.println("trying to connect to zabbix");
					System.out.println("instance ip " + instanceIp);
					String zabi = zabbix_obj.connect();
					System.out.println("zabbi authentification " +  zabi);
