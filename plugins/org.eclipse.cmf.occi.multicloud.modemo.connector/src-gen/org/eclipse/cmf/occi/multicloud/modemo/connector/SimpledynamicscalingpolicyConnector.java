 ...
 * - title: 
 */
public class SimpledynamicscalingpolicyConnector extends org.eclipse.cmf.occi.multicloud.modemo.impl.SimpledynamicscalingpolicyImpl
{
	/**
	...
	// End of user code
	// Start of user code Simpledynamicscalingpolicy_Mixin_stop_action
	/**
	 * Implement OCCI action:
     * - scheme: http://org.eclipse.cmf.occi.multicloud.modemo/scalingpolicy/action#
     * - term: stop
		...
		for (Link link : ec.getLinks()) {
			if (link instanceof Rule) {
				...
				for (MixinBase mixin:mixinsBase) {
					...
					ruleMetric = mixin.getClass().getName().substring(49).toLowerCase();
					System.out.println("mixin metric   " + ruleMetric );
				}
				...
				System.out.println("ruleoperator  " + ruleoperator);
